...
extern char *conf_strdup(const conf_t conf, const char *section, int index, const char *key, const char *defaultval)
{
	const char *v = conf_str(conf, section, index, key, NULL);
	
	if(v == NULL)
	{
		return(defaultval ? strdup(defaultval) : NULL);
	}
