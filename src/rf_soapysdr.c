...
int rf_soapysdr_open(struct rf_t *s, const char *device, unsigned int sample_rate, unsigned int frequency_hz, unsigned int gain, const char *antenna)
{
	struct soapysdr_t *rf;
	SoapySDRKwargs *results;
	size_t length;
	char *sn;
	double fullscale;
	
	rf = calloc(1, sizeof(struct soapysdr_t));
	if(!rf)
	{
		return(-1);
	}
