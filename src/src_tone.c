...
static int _src_tone_read(struct src_tone_t *src, int16_t *audio[2], int audio_step[2])
{
	int i;
...
static int _src_tone_close(struct src_tone_t *src)
{
	free(src->audio);
	free(src);
	return(0);
...
int src_tone_open(struct src_t *s, unsigned int sample_rate, double frequency, double level)
{
	struct src_tone_t *src;
	
	memset(s, 0, sizeof(struct src_t));
	
	src = calloc(1, sizeof(struct src_tone_t));
	if(!src)
	{
		return(-1);
	}
	...
	src->x = 0;
	src->delta = 2.0 * M_PI * frequency / sample_rate;
	src->level = level;
